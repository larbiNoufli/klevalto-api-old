/* tslint:disable max-line-length */
import { ComponentFixture, TestBed, async, inject } from '@angular/core/testing';
import { OnInit } from '@angular/core';
import { DatePipe } from '@angular/common';
import { ActivatedRoute } from '@angular/router';
import { Observable } from 'rxjs/Rx';
import { JhiDateUtils, JhiDataUtils, JhiEventManager } from 'ng-jhipster';
import { SergicTestModule } from '../../../test.module';
import { MockActivatedRoute } from '../../../helpers/mock-route.service';
import { TiersDetailComponent } from '../../../../../../main/webapp/app/entities/tiers/tiers-detail.component';
import { TiersService } from '../../../../../../main/webapp/app/entities/tiers/tiers.service';
import { Tiers } from '../../../../../../main/webapp/app/entities/tiers/tiers.model';

describe('Component Tests', () => {

    describe('Tiers Management Detail Component', () => {
        let comp: TiersDetailComponent;
        let fixture: ComponentFixture<TiersDetailComponent>;
        let service: TiersService;

        beforeEach(async(() => {
            TestBed.configureTestingModule({
                imports: [SergicTestModule],
                declarations: [TiersDetailComponent],
                providers: [
                    JhiDateUtils,
                    JhiDataUtils,
                    DatePipe,
                    {
                        provide: ActivatedRoute,
                        useValue: new MockActivatedRoute({id: 123})
                    },
                    TiersService,
                    JhiEventManager
                ]
            }).overrideTemplate(TiersDetailComponent, '')
            .compileComponents();
        }));

        beforeEach(() => {
            fixture = TestBed.createComponent(TiersDetailComponent);
            comp = fixture.componentInstance;
            service = fixture.debugElement.injector.get(TiersService);
        });

        describe('OnInit', () => {
            it('Should call load all on init', () => {
            // GIVEN

            spyOn(service, 'find').and.returnValue(Observable.of(new Tiers(10)));

            // WHEN
            comp.ngOnInit();

            // THEN
            expect(service.find).toHaveBeenCalledWith(123);
            expect(comp.tiers).toEqual(jasmine.objectContaining({id: 10}));
            });
        });
    });

});
